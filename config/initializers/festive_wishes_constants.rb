module FestiveWishes
  module Festives
    NEW_YEAR = "new_year"
    PONGAL = "pongal"
    REPUBLIC_DAY = "republic_day"
    VALENTINES_DAY = "valentines_day"
    HOLI = "holi"
    TELUGU_NEW_YEAR = "telugu_new_year"
    TAMIL_NEW_YEAR = "tamil_new_year"
    EASTER_SUNDAY = "easter_sunday"
    LABOUR_DAY = "labour_day"
    INDEPENDENCE_DAY = "independence_day"
    RAKSHA_BANDHAN = "raksha_bandhan"
    ONAM = "onam"
    GANESH_CHATHURTHI = "ganesh_chathurthi"
    BAKRID = "bakrid"
    DUSSEHRA = "dussehra"
    DIWALI = "diwali"
    RAMZAN = "ramzan"
    CHRISTMAS = "christmas"
    BIRTHDAY = "birthday"
    ALL = [NEW_YEAR, PONGAL, REPUBLIC_DAY, VALENTINES_DAY, HOLI, TELUGU_NEW_YEAR, TAMIL_NEW_YEAR, EASTER_SUNDAY,
      LABOUR_DAY, INDEPENDENCE_DAY, RAKSHA_BANDHAN, ONAM, GANESH_CHATHURTHI, BAKRID, DUSSEHRA, DIWALI, RAMZAN,
      CHRISTMAS, BIRTHDAY]
  end
end